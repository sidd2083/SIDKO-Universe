...
export default function GoalManager() {
  ...
  const handleAddGoal = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/goals', {
        ...
        credentials: 'include',
        body: JSON.stringify({ title, description, targetDate, progress: 0, milestones: [] }),
      });
      if (!res.ok) throw new Error();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/goals/${id}`, { method: 'DELETE', headers: withAdminHeaders(), credentials: 'include' });
  ...
  const handleAddMilestone = async (goalId: string, milestones: Milestone[]) => {
    const ms = prompt('Enter milestone title:');
    if (!ms) return;
    const newMilestones = [...milestones, { title: ms, done: false }];
    try {
  ...
  const handleToggleMilestone = async (goalId: string, milestones: Milestone[], index: number) => {
    const newMilestones = milestones.map((m, i) => i === index ? { ...m, done: !m.done } : m);
    try {
      await fetch(`/api/goals/${goalId}`, {
        method: 'PATCH',
        headers: withAdminHeaders({ 'Content-Type': 'application/json' }),
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            {goalsLoading ? (
              ...
              <div className="text-center py-16 border border-dashed border-border rounded-2xl text-muted-foreground">No goals yet — add your first one.</div>
            ) : (
                ...
                {goals.map(goal => (
                  <div key={goal.id} className="bg-card border border-border rounded-3xl p-6 shadow-sm relative">
                    ...
                    <div className="pr-12 mb-6">
                      ...
                      {goal.targetDate && <p className="text-xs text-muted-foreground mt-1">Target: {goal.targetDate}</p>}
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => handleAddMilestone(goal.id, goal.milestones || [])}
                      ...
                      <div className="space-y-2">
                        {(goal.milestones || []).map((m, i) => (
                          <div key={i} className="flex items-center gap-3 bg-background border border-border p-3 rounded-xl">
                            ...
                            <span className={`text-sm ${m.done ? 'text-muted-foreground line-through' : 'text-foreground'}`}>{m.title}</span>
                          </div>
                        ))}
